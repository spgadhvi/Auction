...
class ProfileUpdateForm(forms.ModelForm):
    ...
    class Meta:
        model=Profile
        fields = ['image','phone','birthday','address']
...
class ProductForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if start_time and end_time:
            ...
            # Validate the time difference between start time and end time
            time_diff = end_datetime - start_datetime
            if not (timedelta(hours=1) <= time_diff <= timedelta(hours=2)):  # Check if the difference is not between 1 and 2 hours
                raise ValidationError(_("The difference between start time and end time must be between 1 and 2 hours."))
